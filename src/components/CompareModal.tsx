...
export function CompareModal({ isOpen, onClose, listings }: CompareModalProps) {
  ...
  if (!offer1 || !offer2) return null;

  // Determine winners for each category
  const priceWinner = offer1.price.amount < offer2.price.amount ? 1 : offer1.price.amount > offer2.price.amount ? 2 : null;
  const areaWinner = offer1.areaM2 > offer2.areaM2 ? 1 : offer1.areaM2 < offer2.areaM2 ? 2 : null;
  const roomsWinner = offer1.rooms > offer2.rooms ? 1 : offer1.rooms < offer2.rooms ? 2 : null;
  const scoreWinner = offer1.score > offer2.score ? 1 : offer1.score < offer2.score ? 2 : null;
  const distanceWinner = (offer1.distance || 999) < (offer2.distance || 999) ? 1 
